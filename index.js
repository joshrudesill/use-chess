...
BoardCoords.prototype.getSingleRayDepth = function (rayDepth, direction) {
    ...
    if (direction === 1)
        return { x: this.x, y: this.y - rayDepth };
    if (direction === 2)
        return { x: this.x + rayDepth, y: this.y - rayDepth };
    if (direction === 3)
        return { x: this.x - rayDepth, y: this.y };
    if (direction === 4)
        return { x: this.x + rayDepth, y: this.y };
    if (direction === 5)
        return { x: this.x - rayDepth, y: this.y + rayDepth };
            ...
            if (rd[j] !== undefined) {
                ...
                if (access !== null) {
                    ...
                    if (access.color !== turn) {
                        ...
                        const friendlyPiece = new BoardCoords(access.position.x, access.position.y);
                        for (let c = 0; c < 8; c++) {
                            //
                            const pieceAccess = accessBoard(friendlyPiece.getSingleRayDepth(c + 1, j).x, friendlyPiece.getSingleRayDepth(c + 1, j).y);
                            if (pieceAccess === false)
                                break;
                            if (pieceAccess === null)
                                continue;
                            if (pieceAccess !== null) {
                                ...
                                if (pieceAccess.color !== turn) {
                                    ...
                                    if (j === UPPER_LEFT ||
                                        ...
                                        j === LOWER_RIGHT) {
                                        //
                                        if (pieceAccess.stringType === BISHOP ||
                                        ...
                                        j === RIGHT) {
                                        //
                                        if (pieceAccess.stringType === ROOK ||
                                            pieceAccess.stringType === QUEEN) {
                                            // attacking piece, piece is now pinned
                                            ignore.push(j);
                                            pinnedPieces.push({
